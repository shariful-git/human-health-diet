    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            <a href="{{ route('plans.index') }}"
                class="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-extrabold px-4 py-2 rounded-xl transition-all hover:scale-105 inline-flex items-center gap-1">
                ← Back to Plans
            </a>
        </div>
    </x-slot>
        ...
        @foreach ($plan->days as $day)
            <div class="bg-white p-7 rounded-2xl shadow-sm border border-slate-200/80 hover:shadow-md transition-all duration-300 space-y-6">
                                    ...
                                    @forelse($dayFoods as $pFood)
                                        <div class="text-xs font-medium text-slate-700 bg-white p-2.5 rounded-lg border border-slate-200/80 shadow-2xs flex justify-between items-center">
                                            ...
                                            <form action="{{ route('plans.day.removeFood', $pFood->id) }}"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="text-slate-300 hover:text-rose-500 hover:bg-rose-50 p-1 rounded-md transition-colors"
                                                    title="Remove Item">
                                                    ✕
                                                </button>
                                            </form>
                ...
                <form action="{{ route('plans.day.addFood', $day->id) }}" method="POST"
                    ...
                    <div>
                        <button type="submit"
                            class="w-full bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-extrabold h-10 flex items-center justify-center rounded-xl transition-all shadow-xs hover:scale-105">

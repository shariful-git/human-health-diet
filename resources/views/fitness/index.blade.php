<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white p-7 rounded-none shadow-sm border border-slate-200/80 hover:shadow-md transition-shadow flex flex-col justify-between space-y-6">
                <div>
                    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-none bg-cyan-50 text-cyan-600 flex items-center justify-center text-xl font-bold shadow-xs">
                                💧
                            </div>
                            <div>
                        ...
                        <span class="text-xs font-black text-cyan-700 bg-cyan-50 px-3 py-1 rounded-full border border-cyan-200/60">
                            Goal: 3000ml
                        </span>
                    </div>

                    <!-- Visual Water Counter Display -->
                    <div class="text-center my-8 p-6 rounded-none bg-gradient-to-br from-cyan-500/10 via-blue-500/5 to-transparent border border-cyan-200/50">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                    <form action="{{ route('fitness.water.update') }}" method="POST">
                        ...
                        <button type="submit"
                            class="w-full bg-cyan-50 hover:bg-cyan-100/80 text-cyan-700 font-extrabold py-3.5 px-4 rounded-none transition text-center text-xs border border-cyan-200/60 flex items-center justify-center gap-1.5 shadow-2xs">
                    ...
                    <form action="{{ route('fitness.water.update') }}" method="POST">
                        ...
                        <button type="submit"
                            class="w-full bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 text-white font-extrabold py-3.5 px-4 rounded-none transition text-center text-xs flex items-center justify-center gap-1.5 shadow-md shadow-cyan-600/20">
                    ...
                    <form action="{{ route('fitness.water.update') }}" method="POST">
                        ...
                        <input type="hidden" name="amount" value="-250">
                        <button type="submit"
                            class="w-full bg-slate-100 hover:bg-slate-200 text-slate-600 font-extrabold py-3.5 px-4 rounded-none transition text-center text-xs flex items-center justify-center gap-1.5 border border-slate-200">
            ...
            <div class="bg-white p-7 rounded-none shadow-sm border border-slate-200/80 hover:shadow-md transition-shadow space-y-6">
                <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-none bg-amber-50 text-amber-600 flex items-center justify-center text-xl font-bold shadow-xs">
                            🏃‍♂️
                        </div>
                        <div>
                ...
                <!-- Activity Form -->
                <form action="{{ route('fitness.exercise.store') }}" method="POST" class="space-y-4 bg-slate-50/80 p-5 rounded-none border border-slate-200/60">
                    @csrf
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5">Workout Type</label>
                            <select name="exercise_id"
                                class="block w-full rounded-none border-slate-200 bg-white text-xs font-semibold text-slate-800 shadow-2xs focus:border-amber-500 focus:ring-amber-500 py-2.5">
                    ...
                    <button type="submit"
                        class="w-full bg-slate-900 hover:bg-slate-800 text-white font-extrabold py-3 px-4 rounded-none transition text-xs shadow-sm flex items-center justify-center gap-2">
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                        @forelse($exerciseLogs as $log)
                            <div class="flex justify-between items-center bg-slate-50/80 p-3.5 rounded-none border border-slate-200/60 text-xs">
                                <div>
                                ...
                                <div class="flex items-center gap-3">
                                    <span class="font-black text-amber-600 bg-amber-50 border border-amber-200/60 px-2.5 py-1 rounded-none font-mono">
                                        -{{ number_format($log->calculated_calories_burn) }} kcal
                                    </span>
                                    <form action="{{ route('fitness.exercise.destroy', $log->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-slate-300 hover:text-rose-500 hover:bg-rose-50 p-1.5 rounded-none transition-colors" title="Remove Activity">
                                            ✕
                                        </button>
                                    </form>
                                </div>
